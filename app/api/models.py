...
class APIModel:
    ...
    def update_sandbox_keys(user_id, api_key, secret_key):
        db.client_app.update_one({'_id': user_id}, {'$push': {'sandbox_keys': {
                                 'key': api_key, 'secret': secret_key}}})

    @staticmethod
    def insert_sandbox_keys(user_id, api_key, secret_key):
        db.client_app.update_one({'_id': user_id}, {'$set': {'sandbox_keys': [
                                 {'key': api_key, 'secret': secret_key}]}})

    @staticmethod
    def update_live_keys(user_id, api_key, secret_key):
        db.client_app.update_one({'_id': user_id}, {'$push': {'live_keys': {
                                 'key': api_key, 'secret': secret_key}}})

    @staticmethod
    def insert_live_keys(user_id, api_key, secret_key):
        db.client_app.update_one({'_id': user_id}, {'$set': {'live_keys': [
                                 {'key': api_key, 'secret': secret_key}]}})

    @staticmethod
    def delete_sandbox_key(user_id, api_key, secret_key):
        return db.client_app.update_one({'_id': user_id}, {'$pull': {'sandbox_keys': {
                                        'key': api_key, 'secret': secret_key}}})

    @staticmethod
    def delete_live_key(user_id, api_key, secret_key):
        return db.client_app.update_one({'_id': user_id}, {'$pull': {'live_keys': {
                                        'key': api_key, 'secret': secret_key}}})
