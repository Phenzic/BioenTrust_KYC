...
class ClientAdminController:      
    ...
    def update_user_details():
        ...
        ClientAdminModels.update_client_user_status(client_id, user_id, new_status, new_status_description)
        client_profile = ClientAdminModels.get_client_profile(client_id)
    ...
    def dashboard_date():
        ...
        pipeline = [
            {"$match": {"_id": client_id}},
            {"$project": {"date": {"$dateToString": {"format": "%Y-%m-%d", "date": "$requestTime"}}, "status": "$status"}},
            {"$group": {"_id": {"date": "$date", "status": "$status"}, "count": {"$sum": 1}}},
            {"$group": {"_id": "$_id.date", "status_counts": {"$push": {"status": "$_id.status", "count": "$count"}}}}
        ]
        result = list(app.db.client["client_user"].aggregate(pipeline))
        result = {i["_id"]: {"Success": 0, "Failed": 0, **{j["status"]: j["count"] for j in i["status_counts"]}} for i in result}
        return jsonify(result)
